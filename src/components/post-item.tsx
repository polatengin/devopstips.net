  ...
  return (
    <article className="my-6 text-gray-700 dark:text-gray-100" itemScope itemType="http://schema.org/Article">
      <header className="mb-3">
        <Link to={post.fields.slug} className="hover:underline text-3xl">{ post.frontmatter.title }</Link>
      ...
      <Link to={post.fields.slug} className="text-blue-500 text-xs">Read more →</Link>
    </article>
  );
};
